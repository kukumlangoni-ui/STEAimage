...
export default function ImageToAvif() {
  ...
  const processImage = async () => {
    ...
    try {
      ...
      img.onload = async () => {
        ...
        // AVIF support in toDataURL is limited. We'll fallback to WebP if AVIF isn't supported.
        let dataUrl = canvas.toDataURL("image/avif", quality / 100);
    ...
    <div className="mx-auto w-full max-w-7xl px-6 py-12">
      ...
      <div className="mb-12 text-center">
        ...
        <h1 className="text-3xl sm:text-4xl md:text-6xl font-black tracking-tight text-white leading-[0.9] mb-4 break-words">
          Image to <span className="text-amber-500 text-stroke-sm text-transparent">AVIF</span>
        </h1>
        <p className="mx-auto mt-4 max-w-2xl text-lg font-medium text-zinc-400">
          Convert any image to AVIF format for the highest possible compression and quality.
        </p>
      </div>
      ...
      {!file && !resultUrl && status === "idle" && (
        <UploadDropzone
          ...
          title="Upload image to convert to AVIF"
        />
      )}
                ...
                <div className="rounded-xl bg-amber-500/5 p-4 border border-amber-500/10">
                  <div className="flex gap-3">
                    ...
                    <p className="text-[10px] leading-relaxed text-zinc-400">
                      AVIF is the next-generation image format, offering even better compression than WebP.
                    </p>
                  </div>
                </div>
              </div>
      ...
      {(status === "processing" || status === "error") && (
        <ProgressFeedback
          ...
          message={
            status === "processing"
              ? "Converting image to AVIF format..."
      ...
      {resultUrl && status === "success" && (
        <ResultPanel
          title="AVIF Generated!"
          message="Your image has been successfully converted to AVIF format."
