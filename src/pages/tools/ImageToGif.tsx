  ...
  const processImage = async () => {
    ...
    try {
      ...
      img.onload = async () => {
        ...
        setResultUrl(canvas.toDataURL("image/gif"));
        setStatus("success");
    ...
    <div className="mx-auto w-full max-w-7xl px-6 py-12">
      ...
      <div className="mb-12 text-center">
        ...
        <h1 className="text-3xl sm:text-4xl md:text-6xl font-black tracking-tight text-white leading-[0.9] mb-4 break-words">
          Image to <span className="text-amber-500 text-stroke-sm text-transparent">GIF</span>
        </h1>
        <p className="mx-auto mt-4 max-w-2xl text-lg font-medium text-zinc-400">
          Convert any image to GIF format for simple animations and wide compatibility.
        </p>
      </div>
      ...
      {!file && !resultUrl && status === "idle" && (
        <UploadDropzone
          ...
          title="Upload image to convert to GIF"
        />
      )}
                ...
                <div className="rounded-xl bg-amber-500/5 p-4 border border-amber-500/10">
                  <div className="flex gap-3">
                    ...
                    <p className="text-[10px] leading-relaxed text-zinc-400">
                      GIF is a legacy format that supports 256 colors. It's great for simple graphics and animations.
                    </p>
                  </div>
                </div>
              </div>
      ...
      {(status === "processing" || status === "error") && (
        <ProgressFeedback
          ...
          message={
            status === "processing"
              ? "Converting image to GIF format..."
      ...
      {resultUrl && status === "success" && (
        <ResultPanel
          title="GIF Generated!"
          message="Your image has been successfully converted to GIF format."
