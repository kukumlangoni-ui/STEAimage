...
export default function WatermarkImage() {
  ...
  const screenToCanvasPct = (clientX: number, clientY: number): { px: number; py: number } => {
    // Must measure the CANVAS element itself, not the container.
    // The canvas is centered inside the container with padding/flex,
    // so container coords are offset from canvas coords.
    const canvas = canvasRef.current;
