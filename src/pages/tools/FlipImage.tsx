  ...
  const processImage = useCallback(async () => {
    ...
    try {
      ...
      img.onload = () => {
        ...
        if (ctx) {
          ctx.translate(settings.horizontal ? img.width : 0, settings.vertical ? img.height : 0);
          ctx.scale(settings.horizontal ? -1 : 1, settings.vertical ? -1 : 1);
    ...
    <div className="mx-auto w-full max-w-7xl px-6 py-12">
      ...
      <div className="mb-12 text-center">
        ...
        <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-black tracking-tighter text-white leading-[0.9] mb-4 break-words">
          Flip <span className="text-amber-500 text-stroke-sm text-transparent">Image</span>
        </h1>
        <p className="mx-auto mt-4 max-w-2xl text-lg font-medium text-zinc-400">
          Mirror your images horizontally or vertically. Perfect for fixing selfies or creative layouts.
        </p>
      </div>
            ...
            <div className="overflow-hidden rounded-3xl border border-white/5 bg-zinc-900/40 p-4 shadow-2xl backdrop-blur-md">
              <div className="flex justify-center bg-zinc-950/50 rounded-2xl p-4 min-h-[400px] items-center">
                <img
                  ...
                  alt="Preview"
                  style={{ 
                    transform: `scale(${settings.horizontal ? -1 : 1}, ${settings.vertical ? -1 : 1})` 
                  }}
          ...
          <div className="lg:col-span-4 space-y-6">
            <div className="rounded-3xl border border-white/5 bg-zinc-900/40 p-6 shadow-xl backdrop-blur-md">
              ...
              <div className="space-y-4">
                <button
                  onClick={() => setSettings({ ...settings, horizontal: !settings.horizontal })}
                  className={`flex w-full items-center justify-center gap-3 rounded-xl border p-4 text-sm font-bold transition ${
                    settings.horizontal ? "border-amber-500 bg-amber-500/10 text-amber-400" : "border-white/5 bg-zinc-950 text-zinc-400 hover:bg-zinc-900"
                  }`}
                >
                ...
                <button
                  onClick={() => setSettings({ ...settings, vertical: !settings.vertical })}
                  className={`flex w-full items-center justify-center gap-3 rounded-xl border p-4 text-sm font-bold transition ${
                    settings.vertical ? "border-amber-500 bg-amber-500/10 text-amber-400" : "border-white/5 bg-zinc-950 text-zinc-400 hover:bg-zinc-900"
                  }`}
                >
                  <FlipHorizontal size={20} className="rotate-90" />
                  Flip Vertical
                </button>
      ...
      {resultUrl && status === "success" && (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="bg-zinc-900/40 border border-white/5 p-6 rounded-3xl backdrop-blur-md">
              <p className="text-[10px] text-zinc-500 uppercase font-black tracking-widest mb-2">Flipped</p>
                  ...
                  {showComparison ? (
                    <div className="relative w-full h-full flex">
                      ...
                      <div className="w-1/2 overflow-hidden relative">
                        <img src={resultUrl} alt="Flipped" className="max-h-[70vh] w-auto object-contain" referrerPolicy="no-referrer" />
