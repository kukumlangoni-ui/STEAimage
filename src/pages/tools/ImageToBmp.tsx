...
export default function ImageToBmp() {
  ...
  const processImage = async () => {
    ...
    try {
      ...
      img.onload = async () => {
        ...
        setResultUrl(canvas.toDataURL("image/bmp"));
        setStatus("success");
    ...
    <div className="mx-auto w-full max-w-7xl px-6 py-12">
      ...
      <div className="mb-12 text-center">
        ...
        <h1 className="text-3xl sm:text-4xl md:text-6xl font-black tracking-tight text-white leading-[0.9] mb-4 break-words">
          Image to <span className="text-amber-500 text-stroke-sm text-transparent">BMP</span>
        </h1>
        <p className="mx-auto mt-4 max-w-2xl text-lg font-medium text-zinc-400">
          Convert any image to BMP format for uncompressed, high-quality bitmap graphics.
        </p>
      </div>
      ...
      {!file && !resultUrl && status === "idle" && (
        <UploadDropzone
          ...
          title="Upload image to convert to BMP"
        />
      )}
                ...
                <div className="rounded-xl bg-amber-500/5 p-4 border border-amber-500/10">
                  <div className="flex gap-3">
                    ...
                    <p className="text-[10px] leading-relaxed text-zinc-400">
                      BMP is an uncompressed format, meaning the file size will likely be much larger than JPG or PNG.
                    </p>
                  </div>
                </div>
              </div>
      ...
      {(status === "processing" || status === "error") && (
        <ProgressFeedback
          ...
          message={
            status === "processing"
              ? "Converting image to BMP format..."
      ...
      {resultUrl && status === "success" && (
        <ResultPanel
          title="BMP Generated!"
          message="Your image has been successfully converted to BMP format."
