...
export default function Home() {
  ...
  const coreTools = [
    ...
    { name: "WEBP Converter", desc: "Badilisha picha kuwa WEBP", icon: FileType2, path: "/tools/to-webp" },
  ];

  const businessTools = [
    { name: "Social Media Resize", desc: "Kwa Instagram, Facebook, Twitter", icon: Share2, path: "/tools/social-media" },
    { name: "Instagram Grid", desc: "Kata picha kwa grid", icon: Crop, path: "/tools/grid-splitter" },
    ...
    { name: "Product Optimizer", desc: "Boresha picha za biashara", icon: Sparkles, path: "/tools/social-media" },
  ];

  const developerTools = [
    { name: "Favicon Generator", desc: "Tengeneza icon ya website", icon: Code, path: "/tools/favicon" },
    ...
    { name: "Format Detector", desc: "Tambua aina ya picha", icon: FileSearch, path: "/tools/format-detector" },
  ];

  const generalTools = [
    { name: "Merge Images", desc: "Unganisha picha pamoja", icon: Layers, path: "/tools/merge" },
    { name: "Image Splitter", desc: "Gawa picha vipande vipande", icon: Crop, path: "/tools/grid-splitter" },
    { name: "Add Text", desc: "Andika maneno kwenye picha", icon: Type, path: "/tools/add-text" },
    { name: "Text to Image", desc: "Geuza maandishi kuwa picha", icon: Type, path: "/tools/text-to-image" },
  ];

  const qrTools = [
    { name: "QR Code Generator", desc: "Tengeneza QR code yako", icon: Code, path: "/tools/qr-generator" },
    { name: "QR Code Scanner", desc: "Soma QR code kutoka picha", icon: FileSearch, path: "/tools/qr-scanner" },
  ];
  ...
  return (
    <div className="w-full">
      ...
      <section className="mx-auto max-w-7xl px-6 py-16 space-y-16">
        ...
        <div>
          <h2 className="mb-6 text-2xl font-black text-white flex items-center gap-3">
            <span className="text-amber-400">📱 QR Tools</span>
            <div className="h-px flex-1 bg-white/5"></div>
          </h2>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {qrTools.map((tool) => <div key={tool.name}><ToolCard tool={tool} /></div>)}
          </div>
        </div>
