...
import { Zap, Lock, Mail, Loader2, UserPlus } from 'lucide-react';
...
export default function Signup() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError('Imetokea hitilafu wakati wa kujisajili. Tafadhali jaribu tena.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      setError('Imeshindwa kujiunga na Google. Tafadhali jaribu tena.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="w-full max-w-md space-y-8 rounded-3xl border border-white/5 bg-zinc-900/50 p-8 backdrop-blur-xl">
        <div className="text-center">
          <div className="mx-auto flex h-20 w-20 items-center justify-center rounded-3xl bg-amber-400 text-zinc-950 mb-6 shadow-xl shadow-amber-500/20">
            <UserPlus size={40} className="fill-zinc-950" />
          </div>
          <h2 className="text-3xl font-black text-white">Tengeneza Akaunti</h2>
          <p className="mt-2 text-zinc-400">Jiunge na STEAimage leo</p>
        </div>
        ...
        <form onSubmit={handleSignup} className="mt-8 space-y-6">
          ...
          >
            {loading ? <Loader2 className="animate-spin" size={20} /> : 'Jisajili Sasa'}
          </button>
